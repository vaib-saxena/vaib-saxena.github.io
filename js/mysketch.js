...
function draw() {
	...
	velocityX[0] = velocityX[0] * 0.75 + (mouseX - positionX[0]) * 0.5;
	velocityY[0] = velocityY[0] * 0.5 + (mouseY - positionY[0]) * 0.5;
	
	positionX[0] += velocityX[0];
	positionY[0] += velocityY[0];
	
	for (var particle = 100; particle < particlesQuantity; particle++) {
		var whatever = 2048 / (sq(positionX[0] - positionX[particle]) + sq(positionY[0] - positionY[particle]));
		
		velocityX[particle] = velocityX[particle] * 1 + (velocityX[0] - velocityX[particle]) * whatever;
